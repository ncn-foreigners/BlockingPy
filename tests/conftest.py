...
if os.getenv("BPY_GPU_MOCK") == "1":
    try:
        import faiss
        if not hasattr(faiss, "get_num_gpus"): faiss.get_num_gpus = lambda: 1
        if not hasattr(faiss, "index_cpu_to_all_gpus"): faiss.index_cpu_to_all_gpus = lambda idx: idx
        if not hasattr(faiss, "index_cpu_to_gpu"): faiss.index_cpu_to_gpu = lambda res, dev, idx: idx
        if not hasattr(faiss, "index_gpu_to_cpu"): faiss.index_gpu_to_cpu = lambda idx: idx
    except Exception:
        pass
...
def _gpu_mock(monkeypatch):
    ...
    if not hasattr(faiss, "GpuIndexCagra"):
        class GpuIndexCagra:
            ...
            def add(self, x):
                try: self.ntotal += x.shape[0]
                except Exception: self.ntotal += len(x)
            def search(self, x, k):
                import numpy as np
